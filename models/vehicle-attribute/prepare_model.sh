...
MODEL_URL="https://paddleclas.bj.bcebos.com/models/PULC/inference/vehicle_attribute_infer.tar"
MODEL_SHA256="bba326342be959292ec628473d82a0dd9cea43d115e3b4364b4244a0fd104c8b"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORK_DIR="$(mktemp -d)"
trap 'rm -rf "${WORK_DIR}"' EXIT

if [[ -n "${VEHICLE_ATTRIBUTE_TAR:-}" && -f "${VEHICLE_ATTRIBUTE_TAR}" ]]; then
  cp "${VEHICLE_ATTRIBUTE_TAR}" "${WORK_DIR}/model.tar"
else
